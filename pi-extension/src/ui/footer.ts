...
export function updateFooter(ctx: FooterContext, state: FooterState): void {
  if (state.session) {
    ...
    ctx.ui.setStatus(K_SESSION, `\x1b[36m◎\x1b[0m ${state.session} (${count})`);
  } else {
  ...
  if (state.relayOn) {
    ctx.ui.setStatus(
      K_RELAY,
      state.hasPairings ? "\x1b[32m●\x1b[0m relay" : "\x1b[33m○\x1b[0m relay waiting for pairing",
    );
  } else {
  ...
  if (state.devicePaired) {
    ctx.ui.setStatus(K_PEER, `\x1b[36m◈\x1b[0m ${state.devicePaired}`);
  } else {
